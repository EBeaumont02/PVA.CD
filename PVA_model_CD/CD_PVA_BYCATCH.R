...
plot(age.vec,prob.vec,xlab="Age (yrs)", ylab="Probability of becoming bycatch", type="l")
...
for (h in 1:lharv)  {
  ...
  for (e in 1:iter) {
    ...
    for (i in 1:t) {
      ...
      ## Harvest
      n.mat[, i + 1] <- n.mat[, i + 1] - (harv.p.vec[h] * prob.vec * n.mat[, 1 ]) # Substitute i for 1 in last element for proportional vs. proportional harvest, respectively
...
plot(harv.p.vec, r.mn.harv, type = "l", lwd = 2, ylim = range(0.05,r.mn.harv, r.mn.lo, r.mn.up, -0.2), xlab = "bycatch multiplier on current rate", ylab = "mean long-term r")
abline(h = 0, lty = 2, col = "red")
lines(harv.p.vec, r.mn.lo, lty = 2, lwd = 1)
lines(harv.p.vec, r.mn.up, lty = 2, lwd = 1)
...
constant_bycatch_r_data <- data.frame(harv.p.vec, r.mn.harv,r.mn.lo, r.mn.up)
write.csv(constant_bycatch_r_data, file = "constant_bycatch_r_data.csv", row.names = FALSE)
...
constant_bycatch_r_plot <- ggplot(constant_bycatch_r_data, aes(x = harv.p.vec)) +
  geom_line(aes(y = r.mn.harv), color = "black", size = 0.7) +  # Main line in black
  geom_line(aes(y = r.mn.lo), color = "black", size = 0.7, linetype = "dashed") +  # Lower confidence limit with dashed line
  geom_line(aes(y = r.mn.up), color = "black", size = 0.7, linetype = "dashed") +  # Upper confidence limit with dashed line
  geom_hline(yintercept = 0, color = "red", linetype = "dashed", size = 0.7) +  # Add red dashed line at y = 0
  labs(x = "Bycatch increase", y = "Growth rate (r)") +  # Add x and y axis labels
  ...
  scale_y_continuous(limits = c(min(constant_bycatch_r_data$r.mn.harv, constant_bycatch_r_data$r.mn.lo, constant_bycatch_r_data$r.mn.up, na.rm = TRUE), 0),
                     ...
                     expand = expansion(mult = c(0.05, 0.05))) +  # Slightly stretch y-axis
  scale_x_continuous(limits = c(min(constant_bycatch_r_data$harv.p.vec), max(constant_bycatch_r_data$harv.p.vec)),
                     expand = expansion(mult = c(0, 0.05))) +  # Reduce gap between first data point and y-axis
  theme(axis.text = element_text(color = "black", size = 13),  # Adjust axis text size
...
constant_bycatch_r_plot <- ggplot(constant_bycatch_r_data, aes(x = harv.p.vec)) +
  geom_line(aes(y = r.mn.harv), color = "black", size = 0.8, na.rm = TRUE) +  # Main line in black
  geom_ribbon(aes(ymin = r.mn.lo, ymax = r.mn.up), fill = "grey80", alpha = 0.5) +  # Shaded area for confidence interval
...
constant_bycatch_n_data <- data.frame(harv.p.vec, n.mn.harv,n.mn.lo, n.mn.up)
write.csv(constant_bycatch_n_data, file = "constant_bycatch_n_data.csv", row.names = FALSE)
...
constant_bycatch_n_plot <- ggplot(constant_bycatch_n_data, aes(x = harv.p.vec)) +
  geom_line(aes(y = n.mn.harv), color = "black", size = 0.7) +  # Main line in black
  geom_line(aes(y = n.mn.lo), color = "black", size = 0.7, linetype = "dashed") +  # Lower confidence limit with dashed line
  geom_line(aes(y = n.mn.up), color = "black", size = 0.7, linetype = "dashed") +  # Upper confidence limit with dashed line
  labs(x = "Bycatch increase", y = "Population size (N)") +  # Add x and y axis labels
  theme_classic() +  # Apply classic theme
  scale_y_continuous(limits = c(0, max(constant_bycatch_n_data$n.mn.harv, constant_bycatch_n_data$n.mn.lo, constant_bycatch_n_data$n.mn.up, na.rm = TRUE)),
                     ...
                     expand = expansion(mult = c(0, 0.1))) +  # Slightly stretch y-axis
  scale_x_continuous(limits = c(min(constant_bycatch_n_data$harv.p.vec, na.rm = TRUE), max(constant_bycatch_n_data$harv.p.vec, na.rm = TRUE)),
                     expand = expansion(mult = c(0, 0.05))) +  # Reduce gap between first data point and y-axis
...
constant_bycatch_n_plot <- ggplot(constant_bycatch_n_data, aes(x = harv.p.vec)) +
  geom_line(aes(y = n.mn.harv), color = "black", size = 0.8) +  # Main line in black
  geom_ribbon(aes(ymin = n.mn.lo, ymax = n.mn.up), fill = "grey80", alpha = 0.5) +  # Shaded area for confidence interval
...
proportional_bycatch_r_plot <- ggplot(proportional_bycatch_r_data, aes(x = harv.p.vec)) +
  ...
  scale_y_continuous(limits = c(min(proportional_bycatch_r_data$r.mn.harv, proportional_bycatch_r_data$r.mn.lo, proportional_bycatch_r_data$r.mn.up, na.rm = TRUE), 0),
                     ...
                     expand = expansion(mult = c(0.05, 0.05))) +  # Slightly stretch y-axis
  scale_x_continuous(limits = c(min(proportional_bycatch_r_data$harv.p.vec), max(proportional_bycatch_r_data$harv.p.vec)),
                     expand = expansion(mult = c(0, 0.05))) +  # Reduce gap between first data point and y-axis
  theme(axis.text = element_text(color = "black", size = 13),  # Adjust axis text size
        axis.title = element_text(color = "black", size = 12),
        plot.margin = unit(c(0.5, 0.5, 0.5, 0.5), "cm"))  # Increase margin around the plot to prevent labels being cut off
...
proportional_bycatch_r_plot <- ggplot(proportional_bycatch_r_data, aes(x = harv.p.vec)) +
  ...
  scale_y_continuous(limits = c(min(proportional_bycatch_r_data$r.mn.harv, proportional_bycatch_r_data$r.mn.lo, proportional_bycatch_r_data$r.mn.up, na.rm = TRUE), 0),
                     breaks = pretty(c(min(proportional_bycatch_r_data$r.mn.harv, proportional_bycatch_r_data$r.mn.lo, proportional_bycatch_r_data$r.mn.up, na.rm = TRUE), 0), n = 12),
                     expand = expansion(mult = c(0.05, 0.05))) +  
  scale_x_continuous(limits = c(0, 1),  # Change x-axis limits to 0.12
                     expand = expansion(mult = c(0, 0.05))) +  
  theme(axis.text = element_text(color = "black", size = 13),  
        axis.title = element_text(color = "black", size = 12),
        plot.margin = unit(c(0.5, 0.5, 0.5, 0.5), "cm"))
...
proportional_bycatch_n_plot <- ggplot(proportional_bycatch_n_data, aes(x = harv.p.vec)) +
  ...
  scale_y_continuous(limits = c(0, max(proportional_bycatch_n_data$n.mn.harv, proportional_bycatch_n_data$n.mn.lo, proportional_bycatch_n_data$n.mn.up, na.rm = TRUE)),
                     ...
                     expand = expansion(mult = c(0, 0.1))) +  # Slightly stretch y-axis
  scale_x_continuous(limits = c(min(proportional_bycatch_n_data$harv.p.vec, na.rm = TRUE), max(proportional_bycatch_n_data$harv.p.vec, na.rm = TRUE)),
                     expand = expansion(mult = c(0, 0.05))) +  # Reduce gap between first data point and y-axis
  theme(axis.text = element_text(color = "black", size = 11),  # Adjust axis text size
        axis.title = element_text(color = "black", size = 12),
        plot.margin = unit(c(0.5, 0.5, 0.5, 0.5), "cm"))  # Increase margin around the plot to prevent labels being cut off
