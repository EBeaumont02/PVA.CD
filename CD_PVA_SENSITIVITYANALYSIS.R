...
for (r in 1:num_iterations) {
  ...
  params <- lhs_df[r, ]  # params is a vector of parameters
    ...
    surv.vec <- surv.vec.min + surv.temp
      ...
      itdiv <- iter/10 # Interval for progress reporting
...
install.packages("dismo")  # Install dismo package if not already installed
library(dismo)
...
predictors <- c("fert.min.mult.mod", "b.new.mod", "cat.num.mod", "cat.die.off.mod", "calving.prd.mod", "surv.vec.mod")



brt.pop.rate <- gbm(
  formula = pop_rate ~ fert.min.mult.mod + b.new.mod + cat.num.mod + cat.die.off.mod + calving.prd.mod + surv.vec.mod,
  data = SA.pop.rate,
  distribution = "laplace",  # Set the distribution explicitly
...
head(dat)
sum(is.na(dat$r.mn.harv))

# Display variable importance
summary(brt.pop.rate)

var_importance <- summary(brt.pop.rate)

par(mfrow = c(1, 1)) 

# Create a bar plot of the variable importance
# Using base R's barplot
...
par(mfrow = c(2, 3))  # Adjust layout for 6 predictors
for (i in 1:6) {
  plot(brt.pop.rate, i.var = i, main = names(SA.pop.rate)[i])
}

# Predict on the same data (for demonstration)
predictions <- predict(brt.pop.rate, SA.pop.rate, n.trees = brt.pop.rate$n.trees)

# Inspect the first few predictions
head(predictions)

# Calculate mean squared error (MSE)
mse <- mean((SA.pop.rate$pop_rate - predictions)^2)
print(mse)
...
brt.pop.size <- gbm(
  formula = pop_size ~ fert.min.mult.mod + b.new.mod + cat.num.mod + cat.die.off.mod + calving.prd.mod + surv.vec.mod,
  data = SA.pop.size,
  distribution = "laplace",  # Set the distribution explicitly
  n.trees = 1000,            # Number of trees
  shrinkage = 0.01,          # Learning rate
...
# Display variable importance
summary(brt.pop.size)

var_importance <- summary(brt.pop.size)
...
par(mfrow = c(2, 3))  # Adjust layout for 6 predictors
for (i in 1:6) {
  plot(brt.pop.size, i.var = i, main = names(SA.pop.size)[i])
}
...
for (i in 1:6) {
  print(paste("Plotting predictor", i))  # Confirm loop is running
  plot(brt.pop.size, i.var = i, main = paste("Predictor", i))
}
...
par(mfrow = c(2, 3)) 
for (i in 1:6) {
  print(paste("Plotting predictor", i))
  print(plot(brt.pop.size, i.var = i, main = paste("Predictor", i)))
}
